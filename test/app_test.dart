...
void main() {
  ...
  testWidgets('cart store pickup removes delivery fee', (tester) async {
    ...
    expect(deliveryFeeFor('Today 6 PM - 8 PM', 100, fulfilmentMode), 29);
    expect(deliveryFeeFor('Today 6 PM - 8 PM', 100, 'Store pickup'), 0);
  ...
  testWidgets('pickup pass sheet shows pickup code', (tester) async {
    final order = KiranaOrder(
      id: 'SKPICK27',
      items: [CartLine(products.first, 1)],
      total: products.first.price,
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: OrderHandoffPassSheet(order: order)),
      ),
    );

    expect(find.text('Pickup pass'), findsOneWidget);
    expect(find.text('Pickup code'), findsOneWidget);
    expect(find.text(order.handoffCode), findsOneWidget);
