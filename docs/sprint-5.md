...
- Order records can now copy themselves into the next fulfillment state for local operations and UAT flows.
- Account order cards now include Track, WhatsApp, and Advance status actions.
...
1. Connect the local Advance status action to store/admin order status updates.
